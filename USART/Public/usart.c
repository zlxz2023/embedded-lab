...
u8 USART3_RX_BUF[USART3_REC_LEN]; // 存放读取的数据
u16 USART3_RX_STATA=0; // 15-接受完成标志     14-是否接收0x0D     0~13-接收到的数据字节数
...
void USART3_IRQHandler(){
    ...
    if(USART_GetITStatus(USART3, USART_IT_RXNE)==SET){
        ...
        */
        ...
        if((USART3_RX_STATA & 0x8000)==0){
            if(USART3_RX_STATA & 0x4000){
                if(dat != 0x0A){
                    USART3_RX_STATA=0;
                }else{
                    USART3_RX_STATA |= 0x8000;
                }
            }else{
                if(dat==0x0D)
                    USART3_RX_STATA |= 0x4000;
                else{
                    USART3_RX_BUF[USART3_RX_STATA & 0x3fff]=dat;
                    USART3_RX_STATA++;
                    if(USART3_RX_STATA > USART3_REC_LEN-1){
                        USART3_RX_STATA=0;
                    }
                }
            }
